...
import { principles } from "@/components/process/processData";
...
export default function ProcessPrinciples() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.utils.toArray(".princ-card").forEach((el, i) => {
        gsap.fromTo(
          el,
          { opacity: 0, y: 40 },
          {
    ...
    >
      <div className="mb-12 lg:mb-16">
        ...
        <p className="mt-4 text-textGray text-base md:text-lg max-w-2xl leading-relaxed">
          These aren&apos;t aspirations — they&apos;re the non-negotiable
          standards that govern how we operate on every single engagement.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        {principles.map((item) => (
          <div
            key={item.num}
            className="princ-card rounded-2xl border border-[#222] bg-[#0a0a0a] p-6 lg:p-8 hover:border-[#444] transition-colors group"
          >
